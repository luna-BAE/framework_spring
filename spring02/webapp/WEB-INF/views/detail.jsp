...
$(function() {
	$('form:eq(0)').one('submit', function() {
		var msg = $('.page-header>h1').html();
		msg=msg.replace('상세','수정');
		$('.page-header>h1').html(msg);
		$('form:eq(0) button[type="button"]').off('click',del);
		$('form:eq(0) button[type="button"]').attr('type','reset').text("취소").removeClass('btn-danger').addClass('btn-default');
		$('form:eq(0) input').filter(':gt(0)').removeProp('readonly');
		return false;
	});
	
	$('button[type="button"]').on('click', del);
